...
def build_yt_format_string(has_ffmpeg: bool) -> str:
    ...
    Strategy:
    - ถ้ามี ffmpeg: ใช้ bestvideo*+bestaudio* พร้อม height cap
      wildcard * = yt-dlp เลือก codec เองอิสระ ไม่มีทางขึ้น format error
    - ถ้าไม่มี ffmpeg: ใช้ best* ลดขั้นทีละ step พร้อม fallback "best"
    """
    ...
    if has_ffmpeg:
        # bestvideo*+bestaudio* = wildcard ไม่บังคับ codec/ext, cap ที่ height สูงสุด
...
def download_youtube_file(youtube_id: str, series_id: int) -> str:
    ...
    # ─── เลือก format ตามการตั้งค่าคุณภาพ (ไม่ probe ล่วงหน้า) ─────────────
    # wildcard selector ไม่มีทางขึ้น "Requested format is not available"
    ydl_opts["format"] = build_yt_format_string(has_ffmpeg)
