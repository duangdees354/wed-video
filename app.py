...
def ensure_episode_season_column(conn: sqlite3.Connection):
    """เพิ่มคอลัมน์ season_number ให้ตาราง episodes ถ้ายังไม่มี"""
    cur = conn.execute("PRAGMA table_info(episodes)")
    cols = [row[1] for row in cur.fetchall()]
    if "season_number" not in cols:
        conn.execute("ALTER TABLE episodes ADD COLUMN season_number INTEGER DEFAULT NULL")
        conn.commit()


def ensure_series_episode_label_column(conn: sqlite3.Connection):
    """เพิ่มคอลัมน์ episode_label ให้ตาราง series ถ้ายังไม่มี
    ค่า: 'episodes' | 'seasons' | 'both'  default = 'episodes'
    """
    cur = conn.execute("PRAGMA table_info(series)")
    cols = [row[1] for row in cur.fetchall()]
    if "episode_label" not in cols:
        conn.execute("ALTER TABLE series ADD COLUMN episode_label TEXT DEFAULT 'episodes'")
        conn.commit()
...
def init_db():
    ...
    ensure_episode_season_column(conn)
    ensure_series_episode_label_column(conn)
...
def admin_series():
    ...
    if request.method == "POST":
        ...
        episode_label = request.form.get("episode_label", "episodes").strip()
        if episode_label not in ("episodes", "seasons", "both"):
        ...
        if not title:
            ...
            cur.execute(
                """
                INSERT INTO series (title, description, thumbnail_url, episode_label, created_at)
                VALUES (?, ?, ?, ?, ?)
                """,
                (title, description, None, episode_label, datetime.utcnow().isoformat()),
            )
...
def admin_edit_series(series_id):
    ...
    if request.method == "POST":
        ...
        conn.execute(
            """
            UPDATE series
            SET title = ?, description = ?, thumbnail_url = ?, episode_label = ?
            WHERE id = ?
            """,
            (title, description, thumbnail_value, episode_label, series_id),
        )
        conn.commit()
...
def admin_episodes(series_id):
    ...
    if request.method == "POST":
        ...
        episode_type = request.form.get("episode_type", "normal")  # normal | season
        title = request.form.get("title", "").strip()
        description = request.form.get("description", "").strip()
        episode_number_raw = request.form.get("episode_number", "").strip()
        season_number_raw = request.form.get("season_number", "").strip()
        thumbnail_url_input = request.form.get("thumbnail_url", "").strip()
        cover_file = request.files.get("cover_file")

        episode_number = int(episode_number_raw) if episode_number_raw.isdigit() else None
        season_number = int(season_number_raw) if (episode_type == "season" and season_number_raw.isdigit()) else None
        ...
        cur.execute(
            ...
            INSERT INTO episodes (
                id, series_id, title, description, episode_number, season_number,
...
def admin_edit_episode(episode_id):
    ...
    if request.method == "POST":
        ...
        episode_number_raw = request.form.get("episode_number", "").strip()
        season_number_raw = request.form.get("season_number", "").strip()
        episode_type = request.form.get("episode_type", "normal")
        ...
        season_number = None
        if episode_type == "season" and season_number_raw:
            try:
                season_number = int(season_number_raw)
            except ValueError:
                flash("เลขซีซั่นต้องเป็นตัวเลข", "error")
                conn.close()
                return redirect(url_for("admin_edit_episode", episode_id=episode_id))
        ...
        conn.execute(
            """
            UPDATE episodes
            SET title = ?, description = ?, episode_number = ?, season_number = ?,
                source_type = ?, video_url = ?, drive_id = ?, file_path = ?
            WHERE id = ?
            """,
            (
