...
def ensure_episode_status_note_column(conn: sqlite3.Connection):
    """เพิ่มคอลัมน์ status_note ให้ตาราง episodes ถ้ายังไม่มี
    ใช้เก็บข้อความแจ้งเตือนเกี่ยวกับคุณภาพวิดีโอ YouTube"""
    cur = conn.execute("PRAGMA table_info(episodes)")
    cols = [row[1] for row in cur.fetchall()]
    if "status_note" not in cols:
        conn.execute("ALTER TABLE episodes ADD COLUMN status_note TEXT")
        conn.commit()
...
def init_db():
    ...
    ensure_episode_status_note_column(conn)
...
def get_youtube_download_mode() -> str:
    """ดึงโหมดดาวน์โหลด YouTube จาก settings: 'none', 'spoof', 'cookies'"""
    return get_setting("yt_download_mode", "none")


# ลำดับความละเอียดที่รองรับ (สูง → ต่ำ)
YT_QUALITY_LADDER = ["4k", "2k", "1080p", "720p", "480p", "360p", "240p", "144p"]
YT_QUALITY_HEIGHT = {
    "4k":    2160,
    "2k":    1440,
    "1080p": 1080,
...
def get_yt_quality_max() -> str:
    """ดึงคุณภาพสูงสุดที่อนุญาตให้ดาวน์โหลด (ค่าเริ่มต้น: 720p)"""
    val = get_setting("yt_quality_max", "720p")
    return val if val in YT_QUALITY_LADDER else "720p"


def get_yt_quality_min() -> str:
    """ดึงคุณภาพต่ำสุดที่อนุญาตให้ดาวน์โหลด (ค่าเริ่มต้น: 240p)"""
    val = get_setting("yt_quality_min", "240p")
    return val if val in YT_QUALITY_LADDER else "240p"


def build_yt_format_string(has_ffmpeg: bool) -> str:
    """
    สร้าง format string สำหรับ yt-dlp ตามการตั้งค่าคุณภาพ
    - ลองดาวน์โหลดตั้งแต่คุณภาพสูงสุดที่กำหนดลงมาทีละขั้น จนถึงต่ำสุดที่อนุญาต
    - ถ้ามี ffmpeg ใช้ bestvideo+bestaudio ที่ความละเอียดสูงสุดก่อน
    - fallback สุดท้ายไม่ระบุ ext เพื่อป้องกัน "Requested format is not available"
    """
    q_max = get_yt_quality_max()
    q_min = get_yt_quality_min()

    idx_max = YT_QUALITY_LADDER.index(q_max)
    idx_min = YT_QUALITY_LADDER.index(q_min)

    # ถ้าตั้งค่าสลับกัน ให้ swap
    if idx_max > idx_min:
        idx_max, idx_min = idx_min, idx_max

    # สร้าง fallback chain จากสูงสุด → ต่ำสุด
    steps = YT_QUALITY_LADDER[idx_max: idx_min + 1]   # เช่น ["1080p","720p","480p","360p","240p"]

    parts = []
    if has_ffmpeg:
        # bestvideo+bestaudio สำหรับขั้นสูงสุด (ลอง mp4 ก่อน fallback เป็น format อื่น)
        h_top = YT_QUALITY_HEIGHT[steps[0]]
        parts.append(
            f"bestvideo[ext=mp4][height<={h_top}]+bestaudio[ext=m4a]"
            f"/bestvideo[height<={h_top}]+bestaudio"
        )

    # pre-merged MP4 ทีละขั้น แล้ว fallback เป็น format ใดก็ได้ที่ขั้นนั้น
    for q in steps:
        h = YT_QUALITY_HEIGHT[q]
        parts.append(f"best[ext=mp4][height<={h}]")
        parts.append(f"best[height<={h}]")

    # fallback สุดท้าย — ไม่ระบุ ext หรือ height เลย
    parts += ["best[ext=mp4]", "best"]
    return "/".join(parts)


def check_yt_video_quality(youtube_id: str, ydl_opts_base: dict) -> int | None:
    """
    ตรวจสอบความละเอียดสูงสุดของวิดีโอจาก YouTube โดยไม่ดาวน์โหลด
    คืนค่า height (int) หรือ None ถ้าตรวจไม่ได้
    """
    import yt_dlp
    opts = {**ydl_opts_base, "skip_download": True, "quiet": True, "no_warnings": True}
    try:
...
COOKIES_FILE_PATH = os.path.join(DATA_DIR, "yt_cookies.txt")


def resolve_cookies_path() -> str | None:
    """หา cookies ตามลำดับ:
    1. Service Variable  YT_COOKIES_CONTENT (เนื้อหา Netscape cookies โดยตรง) หรือ YT_COOKIES_PATH (path)
    2. ไฟล์ yt_cookies.txt ใน DATA_DIR
    3. ค่าที่บันทึกใน DB settings (yt_cookies_text)
    คืนค่า path ของไฟล์ cookies ที่ใช้งานได้ หรือ None
    """
    # 1. Environment variable — เนื้อหา cookies โดยตรง
    env_content = os.environ.get("YT_COOKIES_CONTENT", "").strip()
    if env_content:
        tmp_path = os.path.join(DATA_DIR, "_env_cookies.txt")
        with open(tmp_path, "w", encoding="utf-8") as f:
            f.write(env_content)
        return tmp_path

    # 1b. Environment variable — path ของไฟล์
    env_path = os.environ.get("YT_COOKIES_PATH", "").strip()
    if env_path and os.path.exists(env_path):
        return env_path

    # 2. ไฟล์ yt_cookies.txt
    if os.path.exists(COOKIES_FILE_PATH):
        return COOKIES_FILE_PATH

    # 3. ค่าใน DB
    db_text = get_setting("yt_cookies_text", "").strip()
    if db_text:
        tmp_path = os.path.join(DATA_DIR, "_db_cookies.txt")
        with open(tmp_path, "w", encoding="utf-8") as f:
            f.write(db_text)
        return tmp_path
...
def get_cookies_source() -> str:
    """ตรวจสอบว่า cookies มาจากแหล่งไหนตอนนี้: 'env', 'file', 'db', หรือ 'none'"""
    env_content = os.environ.get("YT_COOKIES_CONTENT", "").strip()
    if env_content:
        return "env"
    env_path = os.environ.get("YT_COOKIES_PATH", "").strip()
...
def download_youtube_file(youtube_id: str, series_id: int) -> str:
    ...
    รองรับ 3 โหมด (ตามค่า yt_download_mode ใน settings):
      none    — ดาวน์โหลดปกติไม่มีตัวช่วยพิเศษ
      spoof   — ปลอม User-Agent เป็น iOS Safari / Android Chrome
      cookies — ใช้ cookies file เพื่อผ่านการตรวจสอบ

    คุณภาพวิดีโอ (ตามค่า yt_quality_max / yt_quality_min ใน settings):
      - ลองดาวน์โหลดตั้งแต่คุณภาพสูงสุดที่กำหนด ลงมาทีละขั้นจนถึงต่ำสุดที่อนุญาต
      - ถ้าวิดีโอมีความละเอียดต่ำกว่า min → แจ้งเตือนและดาวน์โหลดสิ่งที่ดีที่สุดที่มี
      - ถ้าวิดีโอมีความละเอียดสูงกว่า max → แจ้งเตือนและ cap ที่ระดับ max
    """
    import yt_dlp
    import shutil
    import logging
    ...
    # ปรับตามโหมดที่เลือกไว้
    dl_mode = get_youtube_download_mode()
    ...
    elif dl_mode == "cookies":
        cookies_path = resolve_cookies_path()
        if cookies_path:
            ydl_opts["cookiefile"] = cookies_path

    # ─── ตรวจสอบความละเอียดสูงสุดของวิดีโอก่อนดาวน์โหลด ────────────────────
    q_max = get_yt_quality_max()
    q_min = get_yt_quality_min()
    h_max = YT_QUALITY_HEIGHT[q_max]
    h_min = YT_QUALITY_HEIGHT[q_min]

    video_height = check_yt_video_quality(youtube_id, ydl_opts)

    if video_height is not None:
        if video_height < h_min:
            # วิดีโอมีคุณภาพต่ำกว่าที่กำหนด — ดาวน์โหลดสิ่งที่ดีที่สุดที่มี
            logger.warning(
                "[YT] วิดีโอ %s มีคุณภาพสูงสุดแค่ %dp (ต่ำกว่าขั้นต่ำที่ตั้งไว้ %s) "
                "— จะดาวน์โหลดคุณภาพสูงสุดที่มี",
                youtube_id, video_height, q_min,
            )
            # ใช้ fallback ที่ดีที่สุดของวิดีโอนั้น
            if has_ffmpeg:
                fmt = (
                    f"bestvideo[ext=mp4][height<={video_height}]+bestaudio[ext=m4a]"
                    f"/best[ext=mp4][height<={video_height}]/best[ext=mp4]/best"
                )
            else:
                fmt = f"best[ext=mp4][height<={video_height}]/best[ext=mp4]/best"
        elif video_height > h_max:
            # วิดีโอมีคุณภาพสูงกว่า max — cap ที่ระดับ max
            logger.info(
                "[YT] วิดีโอ %s มีความชัด %dp แต่จะดาวน์โหลดแค่ %s ตามที่ตั้งไว้",
                youtube_id, video_height, q_max,
            )
            fmt = build_yt_format_string(has_ffmpeg)
        else:
            # อยู่ในช่วงที่กำหนด — ใช้ ladder ปกติ
            fmt = build_yt_format_string(has_ffmpeg)
    else:
        # ตรวจไม่ได้ — ใช้ ladder ปกติ
        fmt = build_yt_format_string(has_ffmpeg)
    ...
    for alt_path in (output_template + ".mp4", output_template.replace(".mp4", ".webm")):
...
def _background_youtube_download(episode_id: int, youtube_id: str, series_id: int):
    ...
    - ถ้าวิดีโอมีความละเอียดต่ำกว่า min จะบันทึก status_note เตือน
    - ถ้าวิดีโอมีความละเอียดสูงกว่า max จะบันทึก status_note แจ้ง
    """
    import shutil
    with app.app_context():
        try:
            # ตรวจสอบ quality ก่อนดาวน์โหลด เพื่อสร้าง status_note
            status_note = None
            q_max = get_yt_quality_max()
            q_min = get_yt_quality_min()
            h_max = YT_QUALITY_HEIGHT[q_max]
            ...
            # สร้าง ydl_opts เบื้องต้นเพื่อตรวจ quality
            ...
            video_height = check_yt_video_quality(youtube_id, probe_opts)
            if video_height is not None:
                if video_height < h_min:
                    status_note = (
                        f"⚠️ วิดีโอนี้มีคุณภาพสูงสุดแค่ {video_height}p "
                        f"(ต่ำกว่าขั้นต่ำที่ตั้งไว้ {q_min}) — ดาวน์โหลดคุณภาพสูงสุดที่มี"
                    )
                elif video_height > h_max:
                    status_note = (
                        f"ℹ️ วิดีโอนี้มีความชัด {video_height}p "
                        f"แต่จะดาวน์โหลดแค่ {q_max} ตามที่ตั้งไว้"
                    )
            ...
            conn.execute(
                ...
                WHERE id = ?
                """,
                (rel_path, status_note, episode_id),
            )
            conn.commit()
            conn.close()
        except Exception:
...
def admin_settings():
    ...
    if request.method == "POST":
        ...
        if action == "check_cookies":
            # ตรวจสอบว่า cookies ใช้งานได้จริงหรือเปล่า
            import yt_dlp
            cookies_path = resolve_cookies_path()
            cookies_src = get_cookies_source()
            if not cookies_path:
                flash("⚠️ ไม่พบ cookies จากแหล่งใดเลย กรุณาตั้งค่า cookies ก่อน", "error")
            else:
                try:
                    ydl_opts = {
                        "quiet": True,
                    ...
                    if info:
                        flash(f"✅ Cookies ใช้งานได้ปกติ (แหล่ง: {cookies_src})", "success")
                    else:
                        flash("⚠️ Cookies โหลดได้แต่ตรวจสอบข้อมูลวิดีโอไม่สำเร็จ", "error")
                except Exception as e:
                    flash(f"❌ Cookies ใช้งานไม่ได้: {str(e)[:120]}", "error")
            return redirect(url_for("admin_settings") + "#yt-cookies-section")

        # -- action == save_settings (ค่าเริ่มต้น) --
        has_error = False
        ...
        yt_mode = request.form.get("yt_download_mode", "none").strip()
        if yt_mode in ("none", "spoof", "cookies"):
            set_setting("yt_download_mode", yt_mode)

        # -- YouTube quality limits --
        yt_q_max = request.form.get("yt_quality_max", "720p").strip()
        yt_q_min = request.form.get("yt_quality_min", "240p").strip()
        if yt_q_max in YT_QUALITY_LADDER:
            set_setting("yt_quality_max", yt_q_max)
        if yt_q_min in YT_QUALITY_LADDER:
            set_setting("yt_quality_min", yt_q_min)

        # -- Cookies source preference --
        cookies_source_pref = request.form.get("yt_cookies_source", "env").strip()
        if cookies_source_pref in ("env", "file", "db"):
            set_setting("yt_cookies_source", cookies_source_pref)

        # -- Cookies text (จาก textarea) --
        cookies_text = request.form.get("yt_cookies_text", "").strip()
        set_setting("yt_cookies_text", cookies_text)

        if not has_error:
            flash("บันทึกการตั้งค่าเรียบร้อยแล้ว", "success")
        return redirect(url_for("admin_settings"))

    cookies_source_now = get_cookies_source()
    settings = {
        ...
        "cookies_source_now":   cookies_source_now,
        "yt_quality_max":       get_yt_quality_max(),
        "yt_quality_min":       get_yt_quality_min(),
        "yt_quality_ladder":    YT_QUALITY_LADDER,
    }
...
def admin_episodes(series_id):
    ...
    if request.method == "POST":
        ...
        if source_type == "gdrive":
            ...
            # บันทึกโหมดที่เลือกในฟอร์มก่อนเริ่ม thread
            ep_yt_mode = request.form.get("yt_download_mode", "").strip()
            if ep_yt_mode in ("none", "spoof", "cookies"):
                set_setting("yt_download_mode", ep_yt_mode)
            t = threading.Thread(
...
def admin_retry_download(episode_id):
    ...
    if not episode or episode["source_type"] not in ("gdrive", "youtube") or not episode["drive_id"]:
        flash("ไม่สามารถ retry ได้ เนื่องจากไม่ใช่ตอนประเภท Google Drive หรือ YouTube", "error")
    ...
    if episode["source_type"] == "youtube":
        ...
        flash("เริ่มดาวน์โหลดวิดีโอจาก YouTube ใหม่อีกครั้งแล้ว", "info")
    else:
        t = threading.Thread(
            target=_background_drive_download,
            args=(episode_id, episode["drive_id"], episode["series_id"]),
...
# ---------- ดาวน์โหลด MP4 / MP3 สำหรับแอดมิน ----------

@app.route("/admin/episodes/<int:episode_id>/download/mp4")
def admin_download_episode_mp4(episode_id):
    """ให้แอดมินดาวน์โหลดไฟล์ MP4 ของตอนนั้นลงเครื่องโดยตรง"""
    if not admin_required():
        return redirect(url_for("admin_login"))
    ...
    if not file_path:
        flash("ตอนนี้ไม่มีไฟล์วิดีโอเก็บไว้บนเซิร์ฟเวอร์", "error")
        return redirect(url_for("admin_edit_episode", episode_id=episode_id))

    abs_path = resolve_video_path(file_path)
    if not abs_path or not os.path.isfile(abs_path):
        flash("ไม่พบไฟล์วิดีโอบนเซิร์ฟเวอร์ (อาจถูกลบไปแล้ว)", "error")
        return redirect(url_for("admin_edit_episode", episode_id=episode_id))

    ep_title = ep["title"] or f"episode_{episode_id}"
    safe_title = "".join(c if c.isalnum() or c in " _-" else "_" for c in ep_title).strip()
    ep_num = ep["episode_number"]
    if ep_num:
        download_name = f"ep{ep_num}_{safe_title}.mp4"
    else:
...
@app.route("/admin/episodes/<int:episode_id>/download/mp3")
def admin_download_episode_mp3(episode_id):
    """แปลงไฟล์วิดีโอเป็น MP3 แล้วให้แอดมินดาวน์โหลดลงเครื่อง (ต้องมี ffmpeg)"""
    if not admin_required():
        return redirect(url_for("admin_login"))

    import shutil as _shutil, tempfile, subprocess

    # ── helper: ส่ง error page โดยตรง (ไม่ redirect) เพื่อป้องกันมือถือดาวน์โหลด HTML มาเป็นไฟล์
    def _err_page(msg: str):
        back_url = url_for("admin_episodes", series_id=_series_id) if _series_id else url_for("admin_series")
        html = f"""<!doctype html>
<html lang="th"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>เกิดข้อผิดพลาด</title>
<style>
  body{{font-family:sans-serif;background:#0b0f1a;color:#e0e6f8;
        display:flex;flex-direction:column;align-items:center;
        justify-content:center;min-height:100vh;padding:1.5rem;box-sizing:border-box;}}
  .box{{background:#12172a;border:1px solid #3a2020;border-radius:12px;
        padding:2rem 2.5rem;max-width:520px;width:100%;text-align:center;}}
  h2{{color:#f87171;margin-bottom:1rem;}}
  p{{color:#c0c8e0;line-height:1.6;margin-bottom:1.5rem;}}
  a{{display:inline-block;padding:.65rem 1.5rem;background:#1e3a8a;color:#bfdbfe;
     border-radius:8px;text-decoration:none;font-weight:600;}}
  a:hover{{background:#1e40af;}}
</style></head><body>
<div class="box">
  <h2>❌ ดาวน์โหลด MP3 ไม่สำเร็จ</h2>
  <p>{msg}</p>
  <a href="{back_url}">← ย้อนกลับ</a>
    ...
    if ep is None:
        return _err_page("ไม่พบตอนนี้ในระบบ")

    _series_id = ep["series_id"]

    file_path = ep["file_path"] if "file_path" in ep.keys() else None
    if not file_path:
        return _err_page("ตอนนี้ไม่มีไฟล์วิดีโอเก็บไว้บนเซิร์ฟเวอร์")

    abs_path = resolve_video_path(file_path)
    if not abs_path or not os.path.isfile(abs_path):
        return _err_page("ไม่พบไฟล์วิดีโอบนเซิร์ฟเวอร์ (อาจถูกลบไปแล้ว)")

    # ── หา ffmpeg: ลอง shutil.which ก่อน แล้ว fallback ไปที่ path อื่นบน Nix/Railway
    ...
    if not ffmpeg_bin:
        ...
        # ค้นหาใน /nix/store แบบ glob
        try:
            import glob
            nix_hits = glob.glob("/nix/store/*/bin/ffmpeg")
            candidates.extend(nix_hits)
        except Exception:
            pass
        for c in candidates:
    ...
    if not ffmpeg_bin:
        return _err_page(
            "ไม่พบ <strong>ffmpeg</strong> บนเซิร์ฟเวอร์<br>"
            "กรุณาเพิ่ม <code>ffmpeg</code> ใน nixpacks.toml หรือ Dockerfile แล้ว deploy ใหม่"
        )

    ep_title = ep["title"] or f"episode_{episode_id}"
    safe_title = "".join(c if c.isalnum() or c in " _-" else "_" for c in ep_title).strip()
    ep_num = ep["episode_number"]
    download_name = f"ep{ep_num}_{safe_title}.mp3" if ep_num else f"{safe_title}.mp3"

    tmp_dir = tempfile.mkdtemp(prefix="mp3conv_")
    ...
    try:
        ...
        if result.returncode != 0:
            err_txt = result.stderr.decode("utf-8", errors="replace")[-400:]
            _shutil.rmtree(tmp_dir, ignore_errors=True)
            return _err_page(f"ffmpeg แปลงไฟล์ไม่สำเร็จ:<br><pre style='text-align:left;font-size:.78rem;color:#fca5a5;'>{err_txt}</pre>")
    except subprocess.TimeoutExpired:
        _shutil.rmtree(tmp_dir, ignore_errors=True)
        return _err_page("การแปลงไฟล์ใช้เวลานานเกินไป (timeout 10 นาที)")
    except Exception as ex:
        _shutil.rmtree(tmp_dir, ignore_errors=True)
        return _err_page(f"เกิดข้อผิดพลาด: {ex}")
...
def admin_backup():
    ...
    if request.method == "POST":
        ...
        try:
            if backup_type == "videos":
            ...
            elif backup_type == "other":
                settings_dict = data.get("app_settings", {}) or {}
                ensure_settings_table(conn)

                if mode == "replace":
                    # ล้างการตั้งค่าเดิมทั้งหมดแล้วใส่ใหม่
                ...
                msg = f"คืนค่าการตั้งค่าระบบจากไฟล์สำเร็จแล้ว ({len(settings_dict)} รายการ)"

            else:
                msg = "ไฟล์สำรองประเภทนี้ถูกอ่านสำเร็จ (ไม่มีข้อมูลที่คืนค่าได้เพิ่มเติม)"
...
def admin_backup_download_other():
    ...
    json_bytes = json.dumps(data, ensure_ascii=False, indent=2).encode("utf-8")
    filename = f"settings-{datetime.now().strftime('%Y%m%d')}.json"
    return Response(
        json_bytes,
        mimetype="application/json",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
