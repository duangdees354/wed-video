...
def admin_download_episode_mp3(episode_id):
    ...
    if not shutil.which("ffmpeg"):
        flash("❌ ไม่พบ ffmpeg บนเซิร์ฟเวอร์ — ไม่สามารถแปลงเป็น MP3 ได้ กรุณาติดตั้ง ffmpeg ก่อน", "error")
        return redirect(url_for("admin_edit_episode", episode_id=episode_id))
    ...
    ep_num = ep["episode_number"]
    if ep_num:
        download_name = f"ep{ep_num}_{safe_title}.mp3"
    else:
        download_name = f"{safe_title}.mp3"

    # แปลงในโฟลเดอร์ชั่วคราว
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            flash(f"❌ ffmpeg แปลงไฟล์ไม่สำเร็จ: {err}", "error")
            return redirect(url_for("admin_edit_episode", episode_id=episode_id))
    except subprocess.TimeoutExpired:
        flash("❌ การแปลงไฟล์ใช้เวลานานเกินไป (timeout 10 นาที)", "error")
        return redirect(url_for("admin_edit_episode", episode_id=episode_id))
    except Exception as ex:
        flash(f"❌ เกิดข้อผิดพลาด: {ex}", "error")
        return redirect(url_for("admin_edit_episode", episode_id=episode_id))

    from flask import send_from_directory

    def remove_tmp_after():
    ...
    # ลบไฟล์ชั่วคราวหลัง response ส่งออกไปแล้ว
    resp.call_on_close(remove_tmp_after)
    return resp
