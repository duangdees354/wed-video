...
def build_yt_format_string(has_ffmpeg: bool) -> str:
    ...
    Strategy:
    - ถ้ามี ffmpeg: ใช้ bestvideo+bestaudio (yt-dlp เลือก codec เองไม่บังคับ ext)
      แล้ว merge เป็น mp4 ด้วย merge_output_format
    - ถ้าไม่มี ffmpeg: ใช้ best[height<=X] ลงมาทีละขั้น
    - fallback สุดท้ายคือ "best" เสมอ — ไม่มีทางขึ้น "format not available"
    """
    ...
    steps = YT_QUALITY_LADDER[idx_max: idx_min + 1]
    h_top = YT_QUALITY_HEIGHT[steps[0]]

    if has_ffmpeg:
        # ไม่บังคับ ext — ให้ yt-dlp เลือก stream ที่ดีที่สุดในขอบเขต height แล้ว merge เอง
        fmt = f"bestvideo[height<={h_top}]+bestaudio/best[height<={h_top}]/best"
    else:
        # ไม่มี ffmpeg — ต้องใช้ pre-merged stream, ลดขั้นทีละ step
        parts = []
        for q in steps:
...
def download_youtube_file(youtube_id: str, series_id: int) -> str:
    ...
    if video_height is not None:
        if video_height < h_min:
            ...
            # ใช้ fallback ที่ดีที่สุดของวิดีโอนั้น — ไม่บังคับ ext เพื่อป้องกัน format error
            if has_ffmpeg:
                fmt = (
                ...
                fmt = f"best[height<={video_height}]/best"
